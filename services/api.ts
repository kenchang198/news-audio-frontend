...
const S3_PATH = {
  ...
  playlistById: (id: string) => `/data/playlist_${id}.json`
};
...
export const getEpisodes = async (page: number = 1, limit: number = 10): Promise<{
  ...
  if (USE_MOCK_DATA) {
    ...
    console.log('Using S3 data for episodes');
    try {
      ...
      console.error('Failed to fetch episodes from S3:', error);
      return { episodes: [], totalPages: 0, currentPage: page, totalEpisodes: 0 };
    }
  }
...
export const getEpisode = async (episodeId: string): Promise<Episode | null> => {
  if (USE_MOCK_DATA) {
    ...
    console.log(`Using S3 data for episode ${episodeId}`);
    try {
      ...
      try {
        ...
        console.error(`Failed to fetch S3 episode ${episodeId} or latest episode:`, latestError);
        return null;
      }
    }
  }
...
export const getEpisodePlaylist = async (episodeId: string): Promise<Playlist | null> => {
  if (USE_MOCK_DATA) {
    ...
    console.log(`Using S3 data for playlist ${episodeId}`);
    try {
      // S3からプレイリストを取得
      const response = await axios.get<Playlist>(`${S3_BUCKET_URL}${S3_PATH.playlistById(episodeId)}`);
      return response.data;
    } catch (error) {
      console.warn(`S3 playlist file for ID ${episodeId} not found.`);
      return null;
    }
  }
